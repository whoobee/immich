...
export type JobItem =
  ...
  | { name: JobName.AlbumGeneratorAudioScan; data?: IBaseJob & { force?: boolean } }
...
/** Persisted result of analysing one audio file in the library. Generated by
 * the AlbumGeneratorAudioScan job via the audio-capable Ollama model. The
 * music picker reads these tags to give the LLM a richer catalogue. */
export type AudioAnalysisEntry = {
  /** sha1 of file bytes — used to detect content change and re-analyse */
  sha1: string;
  /** ISO timestamp when this entry was generated */
  analyzedAt: string;
  /** Ollama model that produced it (e.g. "gemma4:e4b") */
  model: string;
  /** One-word mood — calm / energetic / contemplative / playful / etc. */
  mood: string;
  /** Free-form tags the model thinks describe the track */
  tags: string[];
  /** Scenarios from the same vocabulary used for transitions —
   *  scenic, warm, contemplative, nostalgic, action, energetic, playful,
   *  cinematic, urban, natural. Drives the picker. */
  scenarios: string[];
  /** Short prose description from the model — useful for the UI */
  description?: string;
};

/** Persisted scan result keyed by audio filename (basename). */
export type AlbumGeneratorAudioAnalysis = {
  byFilename: Record<string, AudioAnalysisEntry>;
  lastScanAt?: string;
};
...
export interface SystemMetadata extends Record<SystemMetadataKey, Record<string, any>> {
  ...
  [SystemMetadataKey.AlbumGeneratorAudioAnalysis]: AlbumGeneratorAudioAnalysis;
}
